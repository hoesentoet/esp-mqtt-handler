...
#define MQTT_TOPIC_PUBTEST_BOOL "pubTestBool"

#define MQTT_TOPIC_SUBTEST_BOOL "subTestBool"
#define MQTT_TOPIC_SUBTEST_FLOAT "subTestFloat"

MqttHandler mqttHandler(IPAddress(192, 168, 1, 69), "user", "password", "testClient01", true);
...
MqttPubVariable<bool> pubTestBool(false, MQTT_TOPIC_PUBTEST_BOOL);
...
void setup() {
    ...
    Serial.println("Waiting 5s to allow serial monitor to connect for debugging...");
    delay(5000);

    // ── Device info for HA discovery
    // ────────────────────────────────────────── All discoverable entities will
    // be grouped under this device in HA.
    ...
    // **********************************************************************
    Serial.println("Did you forget to add the WiFi connection?");
    // **********************************************************************
    // ************************************************ CONNECT TO WIFI HERE!
    // **********************************************************************
    ...
    subTestBool.onChange([&](MqttSubVariable<bool>& var) {
        ...
        pubTestBool.setValue(var.getValue());
    });
    ...
    mqttHandler.addPubVariable(pubTestBool);
    mqttHandler.addSubVariable(subTestBool);
    mqttHandler.addSubVariable(subTestFloat);

    // ── HA Discovery: pubTestInt → sensor
    // ───────────────────────────────────── A plain integer value without a
    // specific device class — shown as a generic numeric sensor. "state_class:
    // measurement" enables the history graph in HA.
    mqttHandler.addDiscoverable(pubTestInt, MqttDiscoverable{.component = HaComponent::SENSOR,
                                                             .entityId = "pub_test_int",
                                                             .name = "Test Integer",
                                                             .stateClass = "measurement",
                                                             .unitOfMeas = HaUnit::VOLT,
                                                             .icon = "mdi:counter"});

    // ── HA Discovery: pubTestString → sensor
    // ────────────────────────────────── String sensors have no unit or device
    // class; HA displays the raw value.
    mqttHandler.addDiscoverable(pubTestString, MqttDiscoverable{
    ...
    // ───────────────────────────── The ESP only receives this value, so there
    // is no command_topic. HA will display it as ON/OFF; payloadOn/Off default
    // to "true"/"false" which matches the MqttHandler bool serialisation.
    mqttHandler.addDiscoverable(pubTestBool, subTestBool,
                                MqttDiscoverable{
    ...
    // ─────────── Even though this sketch only reads subTestFloat, we register
    // it as a HA "number" entity so the HA UI can send new values to the ESP.
    ...
    mqttHandler.addDiscoverable(subTestFloat, MqttDiscoverable{
                                                  // uses the SubVariable overload
                                                  .component = HaComponent::SENSOR,
