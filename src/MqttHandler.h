...
class MqttSubVariable {
   public:
    ...
    String getTopic(void) { return _topic; }
    bool isTopicStrict(void) { return _strictTopic; }

    void onChange(std::function<void(MqttSubVariable&)> cb) { _onChange = cb; }
...
class MqttPubVariable {
   ...
   private:
    ...
    void _onChange(std::function<void(MqttPubVariable&)> cb) { _onChangeCb = cb; }
};
   ...
   public:
    ...
    void addSubVariable(MqttSubVariable<T>& variable) {
        ...
        this->_subscribe<T>(topic, strictTopic, [&variable, this, topic](T newValue) {
            ...
            if (this->_logging) {
                Serial.println(String("Updated variable '") + variable.getTopic() +
                               String("' to ") + String(variable.getValue()));
            }
        });
    }
