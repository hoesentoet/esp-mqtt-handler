...
void MqttHandler::setDeviceInfo(const String &deviceId,
...
void MqttHandler::_publishDiscoveryConfig(const String &stateTopic,
                                          ...
                                          const MqttDiscoverable &meta)
{
    ...
    String discoveryTopic = String("homeassistant/") + component + "/" + _deviceId + "/" + meta.entityId + "/config";
    ...
    if (!meta.entityCategory.isEmpty())
        doc["entity_category"] = meta.entityCategory;
    ...
    if (!_deviceId.isEmpty())
    {
        ...
        if (!_deviceManufacturer.isEmpty())
            device["manufacturer"] = _deviceManufacturer;
        if (!_swVersion.isEmpty())
    ...
    Serial.printf("Sending discovery (client %s)", _client.connected() ? "connected" : "not connected");
    bool ok = _client.publish(discoveryTopic.c_str(), payload, /*retain=*/true);

    if (_logging)
    {
        Serial.printf("[Discovery] publish -> %s : %s\n",
                      discoveryTopic.c_str(), ok ? "OK" : "FAILED (buffer too small?)");
    }
}
